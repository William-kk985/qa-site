...
**它会断言 7 种语言的产物输出逐位完全相同**，这才是"换语言零成本"的真正证据：
...
| | `.wasm` | `.js` |
|---|---|---|
| 谁走这条 | 能编 wasm 的：MoonBit / Rust / C / C++ / Zig… | 只能编 JS 的：**TypeScript / ReScript** / 手写 JS |
| 跑在哪 | wasm 沙箱，零外部依赖 | **页面里**，和普通脚本同权限 |
| 能碰页面吗 | ❌ 碰不到（这就是它安全的原因） | ✅ DOM、网络、**登录态**都能碰 |
| 怎么加载 | `WebAssembly.instantiate()` | `Blob` + 动态 `import()` |

> 🔒 **一条不能忘的边界**：wasm 插件有沙箱兜着，最坏就是算错数；
...
按扩展名分路（`.js` / `.mjs` 走 JS 后端，其它当 wasm），都会**先试跑一遍，通过才存进本地**：

1. **`.wasm`**：先校验文件头是不是 `\0asm`（不是就提示"这不是 .wasm 文件"），再试着实例化
2. **`.js`**：先试 ES Module 加载，不行再试 CommonJS（两种模块格式都认）
3. **`theme` 和 `hot_score` 一个都没有会直接被拒**，不会存进本地
4. 槽位值越界会被夹回合法范围；返回负数 / `NaN` 表示"这个槽位用默认"
5. 大小上限 512KB；wasm 存 base64，JS 存源码原文
6. 立刻生效，状态行会标明当前是 `WASM` 还是 `JS` 后端
