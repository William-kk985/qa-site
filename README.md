...
| | |
|---|---|
| 文件 | `plugins/hot.wasm`（**只是示例，默认不加载**） |
| 用什么写的 | **MoonBit**（源码 `plugins/moon/hot/hot.mbt`，十几行） |
| 体积 | **297 字节**，零外部依赖 |
| 替换什么 | 热门排序打分：`hot_score(点赞, 回答, 浏览, 距今天数) -> 热度分` |
...
2. **没有导出 `hot_score` 会直接被拒**，不会存进本地
3. 大小上限 512KB（localStorage 放不下更大的）
4. 通过后转成 base64 存进 `localStorage`，立刻生效
