...
## 多语言：WASM 插件位（**只对你自己生效**）

不想写 JavaScript，但想给网站加逻辑？把逻辑编译成 **WebAssembly**，在网站里上传就行 ——
...
| 插件位 | 导出函数 | 作用 |
|---|---|---|
| ① **外观** | `theme(i) -> f64` | 改整站长相：颜色 / 圆角 / 宽度 / 字号…（8 个数字槽位） |
| ② **热门排序** | `hot_score(votes, answers, views, age_days) -> f64` | 替换「热门」标签的打分算法 |

**两个都导出可以，只导出一个也行**（另一个功能就用站点默认）；两个都没有会被拒。

### 关键：为什么 WASM 返回的是「数字」而不是「CSS 字符串」

因为中间夹了一层**通用 JS 桥**（`app.js` 的 `wasmThemeToCss()`），
任何语言编出来的 wasm 都共用它：

```
你的 WASM（Rust / C / Zig / MoonBit / …）
   theme(0) → 152.0   ┐
   theme(1) → 0.62    │   通用 JS 桥：夹范围 → 换算成 CSS → 写进 <style>
   theme(2) → 0.42    ├─►  hsl(152 62% 42%)  →  --primary
   theme(3) → 2.0     │    + px 单位         →  --radius / --maxw / --base-font …
   ……                 ┘
```
...
**所以换语言几乎零成本** —— MoonBit 编的、Rust 编的，`theme(i)` 出来都是 `f64`，JS 一视同仁。
...
| | |
|---|---|
| 文件 | `plugins/example.wasm`（**只是示例，默认不加载**） |
| 用什么写的 | **MoonBit**（源码 `plugins/moon/example/example.mbt`，两个函数共 20 行） |
| 体积 | 几百字节，零外部依赖 |
| 演示什么 | **两样一起**：① 外观改成森林绿 + 直角 + 宽版面；② 热门加时间衰减 |
| 怎么编译 | `bash plugins/build.sh`（需先装 MoonBit 工具链） |
| 怎么用 | 网站 → 🎨 → 「插件」页签 → 「下载示例插件」→「选择 .wasm 文件」上传 |
...
1. 先在浏览器里**试着实例化**这个 wasm
2. **`theme` 和 `hot_score` 一个都没有会直接被拒**，不会存进本地
3. 槽位值越界会被夹回合法范围；返回负数 / `NaN` 表示"这个槽位用默认"
4. 大小上限 512KB（localStorage 放不下更大的）
5. 通过后转成 base64 存进 `localStorage`，立刻生效
