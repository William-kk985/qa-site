...
- **回复（B 站那种一层平铺）暂时没有图片入口**：表结构不限制（`answer_id` 对回复一样适用），
  只是界面上没做 —— 每条回复都塞一个文件选择器会把评论区弄得太重。
  以后要加，`mediaFieldHtml()` 已经是现成的，套到回复框上即可。
