...
> ⚠️ `check-replies.mjs` / `check-avatar.mjs` 需要**先跑过新版 `schema.sql`**
> （`answers.parent_id` / `profiles.avatar_url` / `avatars` 桶都是新增的）。
> 没跑过的话，这两个脚本会在开头明确报「数据库还没升级」并让你先去 SQL Editor
> 整份重跑一遍 —— 不会给你一堆"列不存在"的假失败。
