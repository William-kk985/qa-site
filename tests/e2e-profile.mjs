...
try {
  ...
  /* ⚠️ 按这条问题**自己的 id** 查，不要按标题子串判断：
     数据库是共享的（另一个 agent 跑的 e2e 会造出同名问题），
     「列表里没有『自动化测试』这几个字」是**弱断言** —— 库里只要还有别人一条同名的，
     这里就假失败，而这条问题其实已经删掉了（踩过：同一份代码一次 33/33、一次 32/33，
     失败时前后两条断言全过）。按 id 查是**强断言**，精确到那一条，检出能力反而更强。 */
  const gone = await waitFor(async () => !(await s.ev(
    `!!document.querySelector('#app a[href="#/q/${qid}"]')`)), 10000);
  check('列表里已经没有这条问题（按 qid 精确判断）', gone);
